...
export function AccountsPage({id}: AccountsPageProps) {
    ...
    const transactionsGroupedByAccount = useMemo(() => transactionsInYear.reduce(
    ...
    const accounts = useMemo(() => Object.keys(transactionsGroupedByAccount).map(k => acconutsMap[k]), [acconutsMap, transactionsGroupedByAccount])
...
//net
    ...
    return <Page id={id}><div>{`You used ${accounts.length} different accounts this year`}</div><div>{accounts.map(a => <div>{JSON.stringify(a)}</div>)}</div></Page>
